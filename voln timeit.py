...
def wave_algorithm(edges, start, end):
    ...
    #num_vertices = count_vertices(edges)  # Not used, so removed
    ...
    while True:
        ...
        for v in vertices:
            if visited[v] == step:
                ...
                for edge in edges:
                    if edge[0] == v and edge[1] in vertices and visited[edge[1]] == 0:
                        visited[edge[1]] = step + 1
                        parent[edge[1]] = v
                        new_vertices_found = True
                    if edge[1] == v and edge[0] in vertices and visited[edge[0]] == 0:
...
number = 1000
execution_time1 = timeit.timeit(lambda: wave_algorithm(edges, start, end), number=number)
